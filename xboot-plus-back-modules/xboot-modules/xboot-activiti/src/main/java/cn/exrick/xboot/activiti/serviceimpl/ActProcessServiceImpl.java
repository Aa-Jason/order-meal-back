...
public class ActProcessServiceImpl implements ActProcessService {
    ...
    public ProcessNodeVo getFirstNode(String procDefId) {
        ...
        FlowElement e = null;
        if (startEvent == null) {
            throw new XbootException("未找到该流程起始节点，请确认流程设计是否正确");
        }
        // 判断开始后的流向节点
        ...
        if (e == null) {
            throw new XbootException("未找到该流程起始后的任何节点，请确认流程设计是否正确");
        }
        // 排他、平行网关直接返回
    ...
    public ProcessNodeVo getNode(String nodeId, String taskId, String hisTaskId) {
        ...
        if (actNodeService.hasCustomUser(nodeId)) {
            ...
            if (StrUtil.isBlank(taskId)) {
                throw new XbootException("自选用户节点taskId不能为空");
            }
            ...
            if (StrUtil.isBlank(hisTaskId)) {
                throw new XbootException("连续部门负责人类型节点hisTaskId不能为空");
            }
            // 如果是连续部门负责人类型
